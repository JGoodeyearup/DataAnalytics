...
INSERT INTO Categories (CategoryID, CategoryName, Description)
VALUES(null,'Condiments','Sweet and savory sauces, relishes, spreads, and seasonings');
INSERT INTO Categories (CategoryID, CategoryName, Description) 
VALUES(null,'Confections','Desserts, candies, and sweet breads');
INSERT INTO Categories (CategoryID, CategoryName, Description) 
VALUES(null,'Dairy Products','Cheeses');
INSERT INTO Categories (CategoryID, CategoryName, Description) 
VALUES(null,'Grains/Cereals','Breads, crackers, pasta, and cereal');
INSERT INTO Categories (CategoryID, CategoryName, Description) 
VALUES(null,'Meat/Poultry','Prepared meats');
INSERT INTO Categories (CategoryID, CategoryName, Description) 
VALUES(null,'Produce','Dried fruit and bean curd');
INSERT INTO Categories (CategoryID, CategoryName, Description) 
VALUES(null,'Seafood','Seaweed and fish');
...
INSERT INTO Employees (EmployeeID, LastName, FirstName, Title, TitleOfCourtesy,
  BirthDate, HireDate, Address, City, Region, PostalCode, Country,
  HomePhone, Extension, Notes, ReportsTo, PhotoPath, Salary)
VALUES(null,'King','Robert','Sales Representative','Mr.','1960-05-29','1994-01-02','Edgeham Hollow
...
INSERT INTO Employees (EmployeeID, LastName, FirstName, Title, TitleOfCourtesy,
  BirthDate, HireDate, Address, City, Region, PostalCode, Country,
  HomePhone, Extension, Notes, ReportsTo, PhotoPath, Salary)
VALUES(null,'Callahan','Laura','Inside Sales Coordinator','Ms.','1958-01-09','1994-03-05','4726 - 11th Ave. N.E.','Seattle','WA','98105','USA','(206) 555-1189','2344','Laura received a BA in psychology from the University of Washington.  She has also completed a course in business French.  She reads and writes French.',2
,'http://accweb/emmployees/davolio.bmp','2100.50');
INSERT INTO Employees (EmployeeID, LastName, FirstName, Title, TitleOfCourtesy,
  BirthDate, HireDate, Address, City, Region, PostalCode, Country,
  HomePhone, Extension, Notes, ReportsTo, PhotoPath, Salary)
VALUES(null,'Dodsworth','Anne','Sales Representative','Ms.','1966-01-27','1994-11-15','7 Houndstooth Rd.','London',NULL,'WG2 7LT','UK','(71) 555-4444','452','Anne has a BA degree in English from St. Lawrence College.  She is fluent in French and German.',5
...
-- ============================================================
-- Northwind Date Update Script (2026-02-20)
-- Shifts all dates forward by 27 years
-- Original range: 1996-07-04 to 1998-05-06
-- New range:      2023-07-04 to 2025-05-06
-- ============================================================

-- ------------------------------------------------------------
-- 1. UPDATE TABLE DATA (add 27 years to every date column)
-- ------------------------------------------------------------

-- Orders: OrderDate, RequiredDate, ShippedDate
UPDATE Orders
SET
  OrderDate    = DATE_ADD(OrderDate,    INTERVAL 27 YEAR),
  RequiredDate = DATE_ADD(RequiredDate, INTERVAL 27 YEAR),
  ShippedDate  = DATE_ADD(ShippedDate,  INTERVAL 27 YEAR)
WHERE OrderID > 0;

-- Employees: BirthDate, HireDate
UPDATE Employees
SET
    BirthDate = DATE_ADD(BirthDate, INTERVAL 27 YEAR),
    HireDate  = DATE_ADD(HireDate,  INTERVAL 27 YEAR)
WHERE EmployeeID > 0;

-- ------------------------------------------------------------
-- 2. RECREATE VIEWS WITH UPDATED HARDCODED DATES
--    (Original views reference 1997; updated to 2024)
-- ------------------------------------------------------------

-- Product Sales for 1997 → updated to 2024
DROP VIEW IF EXISTS `Product Sales for 1997`;
CREATE VIEW `Product Sales for 1997` AS
SELECT
    Categories.CategoryName,
    Products.ProductName,
    SUM(CONVERT(`Order Details`.UnitPrice * Quantity * (1 - Discount), DECIMAL(14,2))) AS ProductSales
FROM Categories
    INNER JOIN Products ON Categories.CategoryID = Products.CategoryID
...
-- Category Sales for 1997 → updated to 2024
...
-- Quarterly Orders → updated to 2024
...
-- Sales Totals by Amount → updated to 2024
...
FROM Customers
    ...
    INNER JOIN `Order Subtotals` ON Orders.OrderID = `Order Subtotals`.OrderID
WHERE `Order Subtotals`.Subtotal > 2500
  AND Orders.ShippedDate BETWEEN '2024-01-01' AND '2024-12-31';

-- Sales by Category → updated to 2024
...
SELECT
    ...
    Categories.CategoryName,
    Products.ProductName,
    SUM(CONVERT(`Order Details`.UnitPrice * `Order Details`.Quantity * (1 - `Order Details`.Discount), DECIMAL(14,2))) AS ProductSales
FROM Categories
    INNER JOIN Products ON Categories.CategoryID = Products.CategoryID
    INNER JOIN `Order Details` ON Products.ProductID = `Order Details`.ProductID
    INNER JOIN Orders ON `Order Details`.OrderID = Orders.OrderID
...
UPDATE Employees
SET BirthDate = DATE_ADD(BirthDate, INTERVAL 5 YEAR)
WHERE EmployeeID > 0;
...
-- Check new order date range
-- SELECT
--     MIN(OrderDate)    AS EarliestOrder,
--     MAX(OrderDate)    AS LatestOrder,
--     MIN(ShippedDate)  AS EarliestShipment,
--     MAX(ShippedDate)  AS LatestShipment
-- FROM Orders;
...
SELECT "Please use the refresh button in SCHEMAS panel." AS SUCCESS;